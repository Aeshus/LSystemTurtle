...
Turtle *turtle_create(UINTN screen_width, UINTN screen_height, INTN forward_speed, INTN rotation_speed) {
...
int sin_distance(UINTN distance, INTN deg) {
	return (integer_sine(deg) * distance) / TRIG_SCALE;
}

int cos_distance(UINTN distance, INTN deg) {
	return sin_distance(distance, ((deg - 90) % 360 + 360) % 360);
}
...
void blend_pixel(UINTN screen_width, UINTN screen_height,
                 ...
                 INTN x, INTN y, INTN alpha) {
    if (x < 0 || x >= (INTN)screen_width || y < 0 || y >= (INTN)screen_height || alpha <= 0) {
        return;
    }
...
void turtle_run(UINTN screen_width, UINTN screen_height,
    EFI_GRAPHICS_OUTPUT_BLT_PIXEL *buffer, String *input, INTN forward_speed, INTN rotation_speed) {
    Turtle *t = turtle_create(screen_width, screen_height, forward_speed, rotation_speed);
    ...
    for (INTN i = 0; i < input->len; i++) {
        switch (input->str[i]) {
            case L'F': {
				...
				draw_line(screen_width, screen_height, buffer, t->x, t->y, new->x, new->y);
				turtle_free(t);
				t = new;
                break;
            }
